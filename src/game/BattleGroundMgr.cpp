...
void BattleGroundMgr::DistributeArenaPoints()
{
    ...
    for (std::map<uint32, uint32>::iterator plr_itr = PlayerPoints.begin(); plr_itr != PlayerPoints.end(); ++plr_itr)
    {
        ...
        if (pl)
        {
            CharacterDatabase.PExecute("UPDATE characters SET renaPoints = arenaPoints + '%u' WHERE guid = '%u'", plr_itr->second, plr_itr->first);
        }
        else
        {
            CharacterDatabase.PExecute("UPDATE characters SET renaPoints = arenaPoints + 0 WHERE guid = '%u'", plr_itr->first);
