...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
    ...
    {
        SendNotification(GetDiamondString(LANG_MAIL_SENDER_REQ), sWorld.getConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    {
        SendNotification(GetDiamondString(LANG_MAIL_RECEIVER_REQ), sWorld.getConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (GetSecurity() > SEC_PLAYER && sWorld.getConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr.GetPlayerNameByGUID(sender_guid,sender_name))
                        sender_name = sObjectMgr.GetDiamondStringForDBCLocale(LANG_UNKNOWN);
                }
