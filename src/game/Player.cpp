...
void Player::UpdateCombatSkills(Unit *pVictim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = Diamond::XP::GetGrayLevel(plevel);
...
void Player::SendMessageToSetInRange(WorldPacket *data, float dist, bool self, bool own_team_only)
{
    ...
    Diamond::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
void Player::RewardReputation(Unit *pVictim, float rate)
{
    ...
    if (GetChampioningFaction())
    {
        ...
        if (pMap && pMap->IsDungeon())
        {
            ...
            if (pInstance)
            {
                AccessRequirement const *pAccessRequirement = sObjectMgr.GetAccessRequirement(pInstance->access_id);
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor, bool pvptoken)
{
    ...
    if (honor <= 0)
    {
        ...
        if ( uVictim->GetTypeId() == TYPEID_PLAYER )
        {
            ...
            k_grey = Diamond::XP::GetGrayLevel(k_level);
...
bool Player::CanCompleteQuest( uint32 quest_id )
{
    if ( quest_id )
    {
        ...
        if ( q_status.m_status == QUEST_STATUS_INCOMPLETE )
        {
            ...
            if ( qInfo->HasFlag( QUEST_DIAMOND_FLAGS_DELIVER ) )
            {
            ...
            if ( qInfo->HasFlag(QUEST_DIAMOND_FLAGS_KILL_OR_CAST | QUEST_DIAMOND_FLAGS_SPEAKTO) )
            {
            ...
            if ( qInfo->HasFlag( QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT ) && !q_status.m_explored )
                return false;

            if ( qInfo->HasFlag( QUEST_DIAMOND_FLAGS_TIMED ) && q_status.m_timer == 0 )
                return false;
...
bool Player::CanCompleteRepeatableQuest( Quest const *pQuest )
{
    ...
    if (pQuest->HasFlag( QUEST_DIAMOND_FLAGS_DELIVER) )
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest( Quest const *pQuest, Object *questGiver )
{
    ...
    if ( pQuest->HasFlag(QUEST_DIAMOND_FLAGS_KILL_OR_CAST | QUEST_DIAMOND_FLAGS_SPEAKTO) )
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
    ...
    uint32 qtime = 0;
    if ( pQuest->HasFlag( QUEST_DIAMOND_FLAGS_TIMED ) )
    {
...
void Player::RewardQuest( Quest const *pQuest, uint32 reward, Object* questGiver, bool announce )
{
    ...
    if (pQuest->GetRewHonorableKills())
        RewardHonor(NULL, 0, Diamond::Honor::hk_honor_at_level(getLevel(), pQuest->GetRewHonorableKills()));
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_DIAMOND_FLAGS_TIMED))
    {
        if (msg)
...
void Player::ItemAddedQuestCheck( uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if ( !qInfo || !qInfo->HasFlag(QUEST_DIAMOND_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag( QUEST_DIAMOND_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
...
void Player::SendQuestReward( Quest const *pQuest, uint32 XP, Object * questGiver )
{
    ...
    data << uint32(10*Diamond::Honor::hk_honor_at_level(getLevel(), pQuest->GetRewHonorableKills()));
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    uint32 curTitle = fields[9].GetUInt32();
    ...
    if (result)
    {
        do
        {
            ...
            if (pQuest)
            {
                ...
                if (pQuest->HasFlag(QUEST_DIAMOND_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id) &&  questStatusData.m_status != QUEST_STATUS_NONE)
                {
                    AddTimedQuest(quest_id);
...
bool Player::Satisfy(AccessRequirement const *ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        if (LevelMin || LevelMax || missingItem || missingKey || missingQuest || missingHeroicQuest)
        {
            ...
            {
                if (missingItem)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetDiamondString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr.GetItemPrototype(missingItem)->Name1);
                else if (missingKey)
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetDiamondString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
void Player::HandleStealthedUnitsDetection()
{
    ...
    Diamond::AnyStealthedCheck u_check;
    Diamond::UnitListSearcher<Diamond::AnyStealthedCheck > searcher(this, stealthedUnits, u_check);
...
void Player::UpdateVisibilityOf(T* target, UpdateData& data, std::set<Unit*>& visibleNow)
{
    ...
    else if (target->isVisibleForInState(this,false))
	{
...
bool Player::isHonorOrXPTarget(Unit* pVictim)
{
    ...
    uint8 k_grey  = Diamond::XP::GetGrayLevel(getLevel());
...
bool Player::RewardPlayerAndGroupAtKill(Unit* pVictim)
{
    ...
    if (Group *pGroup = GetGroup())
    {
        ...
        if (member_with_max_level)
        {
            ...
            xp = (PvP || !not_gray_member_with_max_level || GetVehicle()) ? 0 : Diamond::XP::Gain(not_gray_member_with_max_level, pVictim);

            /// skip in check PvP case (for speed, not used)
            bool is_raid = PvP ? false : sMapStore.LookupEntry(GetMapId())->IsRaid() && pGroup->isRaidGroup();
            bool is_dungeon = PvP ? false : sMapStore.LookupEntry(GetMapId())->IsDungeon();
            float group_rate = Diamond::XP::xp_in_group_rate(count,is_raid);
    ...
    {
        xp = (PvP || GetVehicle()) ? 0 : Diamond::XP::Gain(this, pVictim);
