...
void SystemMgr::LoadVersion()
{
    ...
    if (Result)
    {
        ...
        outstring_log("DSCR: Database version is: %s", pFields[0].GetString());
        outstring_log("");
    }
    else
    {
        error_log("DSCR: Missing `version`.`script_version` information.");
        outstring_log("");
    }
}
...
{
    outstring_log("DSCR: Loading Custom Texts...");
    LoadDiamondStrings(WorldDatabase,"custom_texts",TEXT_SOURCE_RANGE*2,1+(TEXT_SOURCE_RANGE*3));

    QueryResult_AutoPtr Result = WorldDatabase.PQuery("SELECT entry, sound, type, language, emote FROM custom_texts");

    outstring_log("DSCR: Loading Custom Texts additional data...");
    ...
    if (Result)
    {
        ...
        do
        {
            ...
            if (iId >= 0)
            {
                error_db_log("DSCR: Entry %i in table `custom_texts` is not a negative value.", iId);
                continue;
            }

            if (iId > TEXT_SOURCE_RANGE*2 || iId <= TEXT_SOURCE_RANGE*3)
            {
                error_db_log("DSCR: Entry %i in table `custom_texts` is out of accepted entry range for table.", iId);
                continue;
            }
            ...
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.uiSoundId))
                    error_db_log("DSCR: Entry %i in table `custom_texts` has soundId %u but sound does not exist.", iId, pTemp.uiSoundId);
            }

            if (!GetLanguageDescByID(pTemp.uiLanguage))
                error_db_log("DSCR: Entry %i in table `custom_texts` using Language %u but Language does not exist.", iId, pTemp.uiLanguage);

            if (pTemp.uiType > CHAT_TYPE_ZONE_YELL)
                error_db_log("DSCR: Entry %i in table `custom_texts` has Type %u but this Chat Type does not exist.", iId, pTemp.uiType);
