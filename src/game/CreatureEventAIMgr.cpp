...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts()
{
    ...
    // Load EventAI Text
    sObjectMgr.LoadDiamondStrings(WorldDatabase,"creature_ai_texts",MIN_CREATURE_AI_TEXT_STRING_ID,MAX_CREATURE_AI_TEXT_STRING_ID);
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!sObjectMgr.GetDiamondStringLocale(i))
            {
...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons()
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!Diamond::IsValidMapCoord(temp.position_x,temp.position_y,temp.position_z,temp.orientation))
            {
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; j++)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_QUEST_EVENT:
                        ...
                        {
                            if (!qid->HasFlag(QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT))
