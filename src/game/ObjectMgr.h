...
typedef UNORDERED_MAP<int32,DiamondStringLocale> DiamondStringLocaleMap;
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadDiamondStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value);
        bool LoadDiamondStrings() { return LoadDiamondStrings(WorldDatabase,"diamond_string",MIN_DIAMOND_STRING_ID,MAX_DIAMOND_STRING_ID); }
        ...
        DiamondStringLocale const* GetDiamondStringLocale(int32 entry) const
        {
            DiamondStringLocaleMap::const_iterator itr = mDiamondStringLocaleMap.find(entry);
            if (itr==mDiamondStringLocaleMap.end()) return NULL;
            return &itr->second;
        }
        const char *GetDiamondString(int32 entry, int locale_idx) const;
        const char *GetDiamondStringForDBCLocale(int32 entry) const { return GetDiamondString(entry,DBCLocaleIndex); }
...
//        GraveYardMap        mGraveYardMap;
    ...
    private:
        ...
        DiamondStringLocaleMap mDiamondStringLocaleMap;
...
// scripting access functions
 bool LoadDiamondStrings(DatabaseType& db, char const* table,int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
 uint32 GetAreaTriggerScriptId(uint32 trigger_id);
 uint32 GetScriptId(const char *name);
