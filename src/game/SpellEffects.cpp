...
void Spell::EffectAddHonor(uint32 /*i*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Diamond::Honor::hk_honor_at_level(unitTarget->getLevel(), damage);
...
void Spell::EffectSanctuary(uint32 /*i*/)
{
    ...
    Diamond::AnyUnfriendlyUnitInObjectRangeCheck u_check(unitTarget, unitTarget, m_caster->GetMap()->GetVisibilityDistance());
    Diamond::UnitListSearcher<Diamond::AnyUnfriendlyUnitInObjectRangeCheck> searcher(unitTarget, targets, u_check);
