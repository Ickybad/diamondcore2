...
ChatCommand * ChatHandler::getCommandTable()
{
    ...
    static ChatCommand reloadCommandTable[] =
    {
        ...
        { "diamond_string",              SEC_ADMINISTRATOR, true,  &ChatHandler::HandleReloadDiamondStringCommand,           "", NULL },
...
const char *ChatHandler::GetDiamondString(int32 entry) const
{
    return m_session->GetDiamondString(entry);
}
...
{
    SendSysMessage(GetDiamondString(entry));
}
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    Diamond::NearestGameObjectCheck check(*pl);
    Diamond::GameObjectLastSearcher<Diamond::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid,uint32 entry)
{
    ...
    if (!obj && sObjectMgr.GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        Diamond::GameObjectWithDbGUIDCheck go_check(*pl,lowguid);
        Diamond::GameObjectSearcher<Diamond::GameObjectWithDbGUIDCheck> checker(pl,obj,go_check);
...
const char *CliHandler::GetDiamondString(int32 entry) const
{
    return sObjectMgr.GetDiamondStringForDBCLocale(entry);
}
...
{
    return GetDiamondString(LANG_CONSOLE_COMMAND);
}
