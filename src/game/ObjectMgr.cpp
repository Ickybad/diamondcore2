...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellPair cell_pair = Diamond::ComputeCellPair(data->posX, data->posY);
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = mQuestTemplates.begin(); iter != mQuestTemplates.end(); ++iter)
    {
        ...
        if (qinfo->QuestFlags & ~QUEST_DIAMOND_FLAGS_DB_ALLOWED)
        {
            sLog.outErrorDb("Quest %u has `SpecialFlags` = %u > max allowed value. Correct `SpecialFlags` to value <= %u",
                qinfo->GetQuestId(),qinfo->QuestFlags,QUEST_DIAMOND_FLAGS_DB_ALLOWED >> 16);
            qinfo->QuestFlags &= QUEST_DIAMOND_FLAGS_DB_ALLOWED;
        }

        if (qinfo->QuestFlags & (QUEST_FLAGS_DAILY | QUEST_FLAGS_WEEKLY))
        {
            if (!(qinfo->QuestFlags & QUEST_DIAMOND_FLAGS_REPEATABLE))
            {
                sLog.outErrorDb("Daily Quest %u not marked as repeatable in `SpecialFlags`, added.",qinfo->GetQuestId());
                qinfo->QuestFlags |= QUEST_DIAMOND_FLAGS_REPEATABLE;
            }
        }
        ...
        for (uint8 j = 0; j < QUEST_ITEM_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_DIAMOND_FLAGS_DELIVER);
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                if (!qinfo->ReqCreatureOrGOId[j])
                {
                    ...
                    if (found)
                    {
                        if (!qinfo->HasFlag(QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT))
                        {
                            sLog.outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and ReqCreatureOrGOId%d = 0, but quest not have flag QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT. Quest flags or ReqCreatureOrGOId%d must be fixed, quest modified to enable objective.",spellInfo->Id,qinfo->QuestId,j+1,j+1);

                            // this will prevent quest completing without objective
                            const_cast<Quest*>(qinfo)->SetFlag(QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT);
                        }
                    }
                    else
                    {
        ...
        for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j )
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_DIAMOND_FLAGS_KILL_OR_CAST | QUEST_DIAMOND_FLAGS_SPEAKTO);
        ...
        if (qinfo->LimitTime)
            qinfo->SetFlag(QUEST_DIAMOND_FLAGS_TIMED);
    }

    // check QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT for spell with SPELL_EFFECT_QUEST_COMPLETE
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!Diamond::IsValidMapCoord(tmp.x,tmp.y,tmp.z,tmp.o))
                {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (!quest->HasFlag(QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT))
                {
                    sLog.outErrorDb("Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, but quest not have flag QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",tablename,tmp.datalong,tmp.id);
...
void ObjectMgr::ReturnOrDeleteOldMails(bool serverUp)
{
    ...
    if (!serverUp)
        CharacterDatabase.PExecute("DELETE FROM mail WHERE expire_time < '" UI64FMTD "' AND has_items = '0'", (uint64)basetime);
    //                                                     0  1           2      3        4         5           6   7       8
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        if (!quest->HasFlag(QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT))
        {
            sLog.outErrorDb("Table `areatrigger_involvedrelation` has record (id: %u) for not quest %u, but quest not have flag QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.",trigger_ID,quest_ID);
...
void ObjectMgr::LoadPointsOfInterest()
{
	...
	do
    {
        ...
        if (!Diamond::IsValidMapCoord(POI.x,POI.y))
        {
...
bool ObjectMgr::LoadDiamondStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value)
{
    ...
    for (DiamondStringLocaleMap::iterator itr = mDiamondStringLocaleMap.begin(); itr != mDiamondStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mDiamondStringLocaleMap.erase(itr++);
        else
            ++itr;
    }
    ...
    do
    {
        ...
        DiamondStringLocale& data = mDiamondStringLocaleMap[entry];
...
const char *ObjectMgr::GetDiamondString(int32 entry, int locale_idx) const
{
    // locale_idx==-1 -> default, locale_idx >= 0 in to idx+1
    // Content[0] always exist if exist DiamondStringLocale
    if (DiamondStringLocale const *msl = GetDiamondStringLocale(entry))
    {
    ...
    if (entry > 0)
        sLog.outErrorDb("Entry %i not found in `diamond_string` table.",entry);
    else
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts.begin(); itrMM != scripts.end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            switch (itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetDiamondStringLocale (itrM->second.dataint))
...
{
    LoadDiamondStrings(WorldDatabase,"db_script_string",MIN_DB_SCRIPT_STRING_ID,MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetDiamondStringLocale(i))
            ids.insert(i);
...
bool LoadDiamondStrings(DatabaseType& db, char const* table,int32 start_value, int32 end_value)
{
    ...
    return sObjectMgr.LoadDiamondStrings(db,table,start_value,end_value);
}
