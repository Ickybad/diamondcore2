...
{
    CellPair pair = Diamond::ComputeCellPair(x, y);
    Cell cell(pair);
    EnsureGridLoaded(cell);
}
...
void Map::MessageBroadcast(Player *player, WorldPacket *msg, bool to_self)
{
    ...
    TypeContainerVisitor<Diamond::MessageDeliverer, WorldTypeMapContainer > message(post_man);
...
void Map::MessageBroadcast(WorldObject *obj, WorldPacket *msg)
{
    ...
    TypeContainerVisitor<Diamond::ObjectMessageDeliverer, WorldTypeMapContainer > message(post_man);
    CellLock<ReadGuard> cell_lock(cell, p);
...
void Map::MessageDistBroadcast(Player *player, WorldPacket *msg, float dist, bool to_self, bool own_team_only)
{
    ...
    TypeContainerVisitor<Diamond::MessageDistDeliverer , WorldTypeMapContainer > message(post_man);
    CellLock<ReadGuard> cell_lock(cell, p);
...
void Map::MessageDistBroadcast(WorldObject *obj, WorldPacket *msg, float dist)
{
    ...
    TypeContainerVisitor<Diamond::ObjectMessageDistDeliverer, WorldTypeMapContainer > message(post_man);
    CellLock<ReadGuard> cell_lock(cell, p);
...
void Map::Update(const uint32 &t_diff)
{
    ...
    // for creature
    TypeContainerVisitor<Diamond::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<Diamond::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                Diamond::DelayedUnitRelocation cell_relocation(cell, pair, *this, GetVisibilityDistance());
                TypeContainerVisitor<Diamond::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<Diamond::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = Diamond::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
void Map::UpdateObjectVisibility( WorldObject* obj, Cell cell, CellPair cellpair)
{
    ...
    TypeContainerVisitor<Diamond::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
    cell.Visit(cellpair, player_notifier, *this, *obj, GetVisibilityDistance());
}

void Map::UpdateObjectsVisibilityFor( Player* player, Cell cell, CellPair cellpair )
{
    Diamond::VisibleNotifier notifier(*player);

    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();
    TypeContainerVisitor<Diamond::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<Diamond::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::PlayerRelocationNotify( Player* player, Cell cell, CellPair cellpair )
{
    ...
    TypeContainerVisitor<Diamond::PlayerRelocationNotifier, GridTypeMapContainer >  p2grid_relocation(relocationNotifier);
    TypeContainerVisitor<Diamond::PlayerRelocationNotifier, WorldTypeMapContainer > p2world_relocation(relocationNotifier);
...
{
    Diamond::CreatureRelocationNotifier relocationNotifier(*creature);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();                                     // not trigger load unloaded grids at notifier call

    TypeContainerVisitor<Diamond::CreatureRelocationNotifier, WorldTypeMapContainer > c2world_relocation(relocationNotifier);
    TypeContainerVisitor<Diamond::CreatureRelocationNotifier, GridTypeMapContainer >  c2grid_relocation(relocationNotifier);
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld.GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                CellPair p(Diamond::ComputeCellPair(summoner->GetPositionX(), summoner->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;
                ...
                TypeContainerVisitor<Diamond::GameObjectSearcher<Diamond::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
            ...
            case SCRIPT_COMMAND_OPEN_DOOR:
            {
                ...
                CellPair p(Diamond::ComputeCellPair(caster->GetPositionX(), caster->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    CellPair p(Diamond::ComputeCellPair(((Unit*)source)->GetPositionX(), ((Unit*)source)->GetPositionY()));
                    Cell cell(p);
                    cell.data.Part.reserved = ALL_DISTRICT;

                    //sLog.outDebug("Attempting to find Creature: Db GUID: %i", step.script->datalong);
                    Diamond::CreatureWithDbGUIDCheck target_check(((Unit*)source), step.script->datalong);
                    Diamond::CreatureSearcher<Diamond::CreatureWithDbGUIDCheck> checker(((Unit*)source), target, target_check);

                    TypeContainerVisitor<Diamond::CreatureSearcher <Diamond::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);
