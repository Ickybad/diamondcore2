...
void CreatureEventAI::ProcessAction(CreatureEventAI_Action const& action, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (action.type)
    {
        ...
        case ACTION_T_SUMMON_GO:
        {
            ...
            if (!pObject)
            {
                sLog.outErrorDb("DSCR: EventAI failed to spawn object %u. Spawn event %d is on creature %d", action.raw.param1, EventId, m_creature->GetEntry());
            }
            break;
        }
...
Unit* CreatureEventAI::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    Diamond::MostHPMissingInRange u_check(m_creature, range, MinHPDiff);
...
void CreatureEventAI::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    Diamond::FriendlyCCedInRange u_check(m_creature, range);
    Diamond::CreatureListSearcher<Diamond::FriendlyCCedInRange> searcher(m_creature, _list, u_check);
...
void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    ...
    Diamond::FriendlyMissingBuffInRange u_check(m_creature, range, spellid);
    Diamond::CreatureListSearcher<Diamond::FriendlyMissingBuffInRange> searcher(m_creature, _list, u_check);
