            ...
            switch (GetSpellProto()->Id)
            {
                ...
                case 51690: 
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(caster, caster, radius);
                        Diamond::UnitListSearcher<Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(caster,targets, u_check);

                        TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
...
void AuraEffect::HandleFeignDeath(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if ( apply )
    {
        ...
        */
        ...
        UnitList targets;
        Diamond::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityDistance());
        Diamond::UnitListSearcher<Diamond::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (!(apply))
    {
        ...
        if (GetSpellProto()->EffectItemType[m_effIndex] == 6265 &&
            (target->getLevel() <= Diamond::XP::GetGrayLevel(caster->getLevel()) ||
