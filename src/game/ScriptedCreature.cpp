...
void ScriptedAI::DoPlaySoundToSet(WorldObject* pSource, uint32 uiSoundId)
{
    ...
    {
        error_log("DSCR: Invalid soundId %u used in DoPlaySoundToSet (Source: TypeId %u, GUID %u)", uiSoundId, pSource->GetTypeId(), pSource->GetGUIDLow());
        return;
    }
...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        error_log("DSCR: DoResetThreat called for creature that either cannot have threat list or has empty threat list (m_creature entry = %d)", m_creature->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float fX, float fY, float fZ, float fO)
{
    ...
    {
        if (pUnit)
            error_log("DSCR: Creature %u (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: %u) to x: %f y:%f z: %f o: %f. Aborted.", m_creature->GetGUID(), m_creature->GetEntry(), pUnit->GetTypeId(), pUnit->GetGUID(), fX, fY, fZ, fO);
        return;
    }
...
Player* ScriptedAI::GetPlayerAtMinimumRange(float fMinimumRange)
{
    ...
    Diamond::PlayerAtMinimumRangeAway check(m_creature, fMinimumRange);
    Diamond::PlayerSearcher<Diamond::PlayerAtMinimumRangeAway> searcher(m_creature, pPlayer, check);
    TypeContainerVisitor<Diamond::PlayerSearcher<Diamond::PlayerAtMinimumRangeAway>, GridTypeMapContainer> visitor(searcher);
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(const uint32 uiDiff)
{
    ...
    switch (m_creature->GetEntry())
    {
        ...
        case NPC_SARTHARION:                                         // sartharion (calculate box)
            ...
            break;
        default:
            error_log("DSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition.", m_creature->GetEntry());
            return false;
    }
