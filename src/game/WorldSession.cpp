...
{
    char const* format = GetDiamondString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetDiamondString( int32 entry ) const
{
    return sObjectMgr.GetDiamondString(entry,GetSessionDbLocaleIndex());
}
