...
bool ChatHandler::HandleDebugEnterVehicle(const char * args)
{
    ...
    else
    {
        ...
        Diamond::AllCreaturesOfEntryInRange check(m_session->GetPlayer(), entry, 20.0f);
        Diamond::CreatureSearcher<Diamond::AllCreaturesOfEntryInRange> searcher(m_session->GetPlayer(), passenger, check);
...
bool ChatHandler::HandleDebugSetValueCommand(const char* args)
{
    ...
    if (isint32)
    {
        ...
        sLog.outDebug(GetDiamondString(LANG_SET_UINT), GUID_LOPART(guid), Opcode, iValue);
    ...
    else
    {
        ...
        sLog.outDebug(GetDiamondString(LANG_SET_FLOAT), GUID_LOPART(guid), Opcode, fValue);
...
bool ChatHandler::HandleDebugGetValueCommand(const char* args)
{
    ...
    if (isint32)
    {
        ...
        sLog.outDebug(GetDiamondString(LANG_GET_UINT), GUID_LOPART(guid), Opcode, iValue);
        PSendSysMessage(LANG_GET_UINT_FIELD, GUID_LOPART(guid), Opcode,    iValue);
    }
    else
    {
        fValue = target->GetFloatValue( Opcode );
        sLog.outDebug(GetDiamondString(LANG_GET_FLOAT), GUID_LOPART(guid), Opcode, fValue);
...
bool ChatHandler::HandleDebugMod32ValueCommand(const char* args)
{
    ...
    sLog.outDebug(GetDiamondString(LANG_CHANGE_32BIT), Opcode, Value);
