...
bool ChatHandler::HandleGameObjectAddCommand(const char* args)
{
    ...
    sLog.outDebug(GetDiamondString(LANG_GAMEOBJECT_CURRENT), gInfo->name, db_lowGUID, x, y, z, o);
...
bool ChatHandler::HandleModifyRepCommand(const char * args)
{
    ...
    if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
    {
        ...
        {
            std::string rank = GetDiamondString(ReputationRankStrIndex[r]);
            if (rank.empty())
                continue;
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetDiamondString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleLookupEventCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < events.size(); ++id )
    {
        ...
        {
            char const* active = activeEvents.find(id) != activeEvents.end() ? GetDiamondString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleEventActiveListCommand(const char* args)
{
    ...
    char const* active = GetDiamondString(LANG_ACTIVE);
...
bool ChatHandler::HandleEventInfoCommand(const char* args)
{
    ...
    char const* activeStr = active ? GetDiamondString(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleLookupTitleCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetDiamondString(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE)==titleInfo->bit_index
                    ? GetDiamondString(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    LocaleConstant loc = GetSessionDbcLocale();
    char const* targetName = target->GetName();
    char const* knownStr = GetDiamondString(LANG_KNOWN);
