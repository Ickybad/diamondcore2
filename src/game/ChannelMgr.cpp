...
ChannelMgr* channelMgr(uint32 team)
{
    if (sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHANNEL))
        return &Diamond::Singleton<AllianceChannelMgr>::Instance();        // cross-faction

    if (team == ALLIANCE)
        return &Diamond::Singleton<AllianceChannelMgr>::Instance();
    if (team == HORDE)
        return &Diamond::Singleton<HordeChannelMgr>::Instance();
