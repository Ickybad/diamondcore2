...
bool ChatHandler::HandleReloadAllCommand(const char*)
{
    ...
    HandleReloadDiamondStringCommand("");
...
bool ChatHandler::HandleReloadDiamondStringCommand(const char*)
{
    sLog.outString( "Re-Loading diamond_string Table!" );
    sObjectMgr.LoadDiamondStrings();
    SendGlobalGMSysMessage("DB table `diamond_string` reloaded.");
    return true;
}
...
bool ChatHandler::HandleAccountSetGmLevelCommand(const char *args)
{
    ...
    uint32 plSecurity = m_session ? accmgr.GetSecurity(m_session->GetAccountId()) : SEC_CONSOLE;
...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target==m_session->GetPlayer() ? GetDiamondString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id,true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target==m_session->GetPlayer() ? GetDiamondString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    sLog.outDetail(GetDiamondString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog.outDetail(GetDiamondString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                {
                    knownStr = GetDiamondString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetDiamondString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                if (rank)
                    ss << GetDiamondString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetDiamondString(LANG_TALENT);
                if (passive)
                    ss << GetDiamondString(LANG_PASSIVE);
                if (learn)
                    ss << GetDiamondString(LANG_LEARN);
                if (known)
                    ss << GetDiamondString(LANG_KNOWN);
                if (active)
                    ss << GetDiamondString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetDiamondString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetDiamondString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetDiamondString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                if (repState)                               // and then target!=NULL also
                {
                    ...
                    std::string rankName = GetDiamondString(ReputationRankStrIndex[rank]);

                    ss << " " << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ")";

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetDiamondString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetDiamondString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetDiamondString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetDiamondString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetDiamondString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetDiamondString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetDiamondString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < MAX_LOCALE)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetDiamondString(LANG_CONTINENT);

                switch (MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetDiamondString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetDiamondString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetDiamondString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetDiamondString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetDiamondString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetDiamondString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetDiamondString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetDiamondString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetDiamondString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetDiamondString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleDebugSet32Bit(const char *args)
{
    ...
    sLog.outDebug(GetDiamondString(LANG_SET_32BIT), Opcode, Value);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetDiamondString(LANG_TALENT);
    char const* passiveStr = GetDiamondString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent?GetDiamondString(LANG_BANINFO_INFINITE):secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetString(), bantime.c_str(), active ? GetDiamondString(LANG_BANINFO_YES):GetDiamondString(LANG_BANINFO_NO), fields[4].GetString(), fields[5].GetString());
    }while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetString(), fields[1].GetString(), permanent ? GetDiamondString(LANG_BANINFO_NEVER):fields[2].GetString(),
        permanent ? GetDiamondString(LANG_BANINFO_INFINITE):secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetString(), fields[5].GetString());
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    CellPair p(Diamond::ComputeCellPair(pl->GetPositionX(), pl->GetPositionY()));
    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Diamond::RespawnDo u_do;
    Diamond::WorldObjectWorker<Diamond::RespawnDo> worker(pl,u_do);

    TypeContainerVisitor<Diamond::WorldObjectWorker<Diamond::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
