...
void WorldSession::HandleMessagechatOpcode( WorldPacket & recv_data )
{
    ...
    else
    {
        ...
        if (!_player->CanSpeak())
        {
            ...
            SendNotification(GetDiamondString(LANG_WAIT_BEFORE_SPEAKING),timeStr.c_str());
            return;
        }
    ...
    if (GetPlayer()->HasAura(1852) && type != CHAT_MSG_WHISPER)
    {
        ...
        SendNotification(GetDiamondString(LANG_GM_SILENCE), GetPlayer()->GetName());
        return;
    }
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetDiamondString(LANG_SAY_REQ), sWorld.getConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetDiamondString(LANG_WHISPER_REQ), sWorld.getConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            ...
            if (!processChatmessageFurtherAfterSecurityChecks(msg, lang))
             ...
             {
                 SendNotification(GetDiamondString(LANG_CHANNEL_REQ), sWorld.getConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                 return;
             }
        ...
        case CHAT_MSG_AFK:
        {
            ...
            if ((msg.empty() || !_player->isAFK()) && !_player->isInCombat() )
            {
                ...
                {
                    if (msg.empty())
                        msg  = GetDiamondString(LANG_PLAYER_AFK_DEFAULT);
                    _player->afkMsg = msg;
                }
                _player->ToggleAFK();
        ...
        case CHAT_MSG_DND:
        {
            ...
            if (msg.empty() || !_player->isDND())
            {
                ...
                {
                    if (msg.empty())
                        msg  = GetDiamondString(LANG_PLAYER_DND_DEFAULT);
                    _player->dndMsg = msg;
                }
                _player->ToggleDND();
...
void WorldSession::HandleTextEmoteOpcode( WorldPacket & recv_data )
{
    ...
    CellPair p = Diamond::ComputeCellPair(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());
    ...
    Diamond::PlayerDistWorker<Diamond::LocalizedPacketDo<Diamond::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld.getConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<Diamond::PlayerDistWorker<Diamond::LocalizedPacketDo<Diamond::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);
