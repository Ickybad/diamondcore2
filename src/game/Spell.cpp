...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    CellPair p(Diamond::ComputeCellPair(m_caster->GetPositionX(), m_caster->GetPositionY()));
    Cell cell(p);
    ...
    Diamond::WorldObjectSearcher<T> searcher(m_caster, result, u_check);

    TypeContainerVisitor<Diamond::WorldObjectSearcher<T>, GridTypeMapContainer > grid_searcher(searcher);
    cell.Visit(p, grid_searcher, *m_caster->GetMap(), *m_caster, max_range);

    if (!result)
    {
        TypeContainerVisitor<Diamond::WorldObjectSearcher<T>, WorldTypeMapContainer > world_searcher(searcher);
            ...
            switch (m_spellInfo->Effect[i])
            {
                case SPELL_EFFECT_DUMMY:
                {
                    switch (m_spellInfo->Id)
                    {
                        ...
                        {
                            WorldObject* result = FindCorpseUsing<Diamond::CannibalizeObjectCheck> ();
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Diamond::SpellNotifierCreatureAndPlayer notifier(m_caster, TagUnitMap, radius, type, TargetType, pos, entry);
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargets TargetType)
{
    switch (TargetType)
    {
        ...
        {
            Unit *target = NULL;
            Diamond::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Diamond::UnitLastSearcher<Diamond::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
        ...
        {
            Unit *target = NULL;
            Diamond::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, range);
            Diamond::UnitLastSearcher<Diamond::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, target, u_check);
            m_caster->VisitNearbyObject(range, searcher);
            return target;
        }
    }
    ...
    else if (pushType)
    {
        ...
        if (targetType == SPELL_TARGETS_ENTRY)
        {
            ...
            if (bounds.first == bounds.second)
            {
                ...
                switch (m_spellInfo->Id)
                {
                    ...
                    {
                        if (WorldObject* result = FindCorpseUsing<Diamond::RaiseDeadObjectCheck> ())
                        {
                            switch (result->GetTypeId())
                            {
                                case TYPEID_UNIT:
                    ...
                    case 51328:
                                ...
                                && m_targets.getUnitTarget()->GetDisplayId() == m_targets.getUnitTarget()->GetNativeDisplayId())))
                        {
                            ...
                            WorldObject* result = FindCorpseUsing <Diamond::ExplodeCorpseObjectCheck> ();
        ...
        else
        {
            switch (cur)
            {
                ...
                case TARGET_OBJECT_AREA_DST:
                {
                    ...
                    Diamond::GameObjectInRangeCheck check(x, y, z, radius + 50);
                    std::list<GameObject*> goList;
                    Diamond::GameObjectListSearcher<Diamond::GameObjectInRangeCheck> searcher(m_caster, goList, check);
        ...
        if (!unitList.empty())
        {
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                Diamond::RandomResizeList(unitList, m_spellValue->MaxAffectedTargets);
            }
            else
            {
                switch (m_spellInfo->Id)
                    ...
                    switch (i_spellST->second.type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            if (i_spellST->second.targetEntry)
                            {
                                ...
                                Diamond::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster,i_spellST->second.targetEntry,range);
                                Diamond::GameObjectLastSearcher<Diamond::NearestGameObjectEntryInObjectRangeCheck> checker(m_caster, p_GameObject,go_check);

                                TypeContainerVisitor<Diamond::GameObjectLastSearcher<Diamond::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
                        ...
                        default:
                        {
                            ...
                            TypeContainerVisitor<Diamond::CreatureLastSearcher<Diamond::NearestCreatureEntryWithLiveStateInObjectRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
SpellCastResult Spell::CheckItems()
{
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        ...
        GameObject* ok = NULL;
        Diamond::GameObjectFocusCheck go_check(m_caster,m_spellInfo->RequiresSpellFocus);
        Diamond::GameObjectSearcher<Diamond::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<Diamond::GameObjectSearcher<Diamond::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
