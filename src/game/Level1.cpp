...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetDiamondString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetDiamondString(LANG_GM_NOTIFY);
    str += args;
...
{
        const char *format = GetDiamondString(entry);
        va_list ap;
        char str [1024];
        va_start(ap, entry);
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetDiamondString(LANG_VISIBLE) : GetDiamondString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleGPSCommand(const char* args)
{
    ...
    CellPair cell_val = Diamond::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
    Cell cell(cell_val);
    ...
    GridPair p = Diamond::ComputeGridPair(obj->GetPositionX(), obj->GetPositionY());
    ...
    sLog.outDebug("Player %s GPS call for %s '%s' (%s: %u):",
        m_session ? GetNameLink().c_str() : GetDiamondString(LANG_CONSOLE_COMMAND),
        (obj->GetTypeId() == TYPEID_PLAYER ? "player" : "creature"), obj->GetName(),
        (obj->GetTypeId() == TYPEID_PLAYER ? "GUID" : "Entry"), (obj->GetTypeId() == TYPEID_PLAYER ? obj->GetGUIDLow(): obj->GetEntry()) );
    sLog.outDebug(GetDiamondString(LANG_MAP_POSITION),
...
bool ChatHandler::HandleNamegoCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(),GetDiamondString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(const char* args)
{
    ...
    sLog.outDetail(GetDiamondString(LANG_CURRENT_ENERGY),chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(const char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        sLog.outDetail(GetDiamondString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0 )
        {
    ...
    sLog.outDetail(GetDiamondString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney() );
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetDiamondString(LANG_ON) : GetDiamondString(LANG_OFF));
        return true;
    }
...
/*    MapEntry const * me = sMapStore.LookupEntry(tele->mapId);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetDiamondString(LANG_OFFLINE), tele->name.c_str());
