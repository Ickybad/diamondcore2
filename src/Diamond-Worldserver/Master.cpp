    ...
    void run ()
    {
        ...
        if (usera)
        {
            ...
            if (!Utility::u2ip (stringip, raip))
                sLog.outError ("Diamond RA can not bind to ip %s", stringip.c_str ());
            else if (RAListenSocket.Bind (raip, raport))
				sLog.outError ("Diamond RA can not bind to port %d on %s", raport, stringip.c_str ());
            else
            {
    ...
    #ifdef WIN32
    {
        ...
        if (Aff > 0)
        {
            ...
            if (GetProcessAffinityMask(hProcess,&appAff,&sysAff))
            {
                ...
                if (!curAff )
                {
                    sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not accessible for Diamondcore. Accessible processors bitmask (hex): %x",Aff,appAff);
                }
                else
                {
...
//        if (Prio && (m_ServiceStatus == -1)/* need set to default process priority class in service mode*/)
        if (Prio)
        {
            if (SetPriorityClass(hProcess,HIGH_PRIORITY_CLASS))
                sLog.outString("DiamondCore process priority class set to HIGH");
            else
                sLog.outError("ERROR: Can't set DiamondCore process priority class.");
            sLog.outString("");
        }
    }
    #endif
